...
def time_operation(
    op_fn: Callable, num_runs: int = 100, warmup: int = 10
) -> Tuple[float, float]:
...
def run_benchmark(test_case: TestCase) -> None:
    ...
    for shape1, shape2 in shapes:
        ...
        unfused_y = ad.layernorm(
            ad.matmul(node_x1, node_x2), normalized_shape=normalized_shape
        )
        ...
        fused_mean, fused_std = time_operation(
            lambda: fused_eval.run({node_x1: x1, node_x2: x2}), test_case.num_runs
        )
        unfused_mean, unfused_std = time_operation(
            lambda: unfused_eval.run({node_x1: x1, node_x2: x2}), test_case.num_runs
        )
        ...
        unfused_mean, unfused_std = time_operation(
            lambda: unfused_grad_eval.run(
                {node_x1: x1, node_x2: x2, node_grad: y_grad}
            ),
            test_case.num_runs,
        )
        print(f"Fused:    {fused_mean*1000:.3f} ms ± {fused_std*1000:.3f} ms")
